...
#endif
    __ branch(lir_cond_belowEqual, T_INT,
#ifdef RISCV64
              left, right,
#endif
              stub); // forward branch
...
#endif
    __ branch(lir_cond_aboveEqual, T_INT,
#ifdef RISCV64
              index, right,
#endif
              stub); // forward branch
...
void LIRGenerator::new_instance(LIR_Opr dst, ciInstanceKlass* klass, bool is_unresolved, LIR_Opr scratch1, LIR_Opr scratch2, LIR_Opr scratch3, LIR_Opr scratch4, LIR_Opr klass_reg, CodeEmitInfo* info) {
  ...
  if (UseFastNewInstance && klass->is_loaded()
    ...
    __ branch(lir_cond_always, T_ILLEGAL,
#ifdef RISCV64
...
void LIRGenerator::do_SwitchRanges(SwitchRangeArray* x, LIR_Opr value, BlockBegin* default_sux) {
  ...
  for (int i = 0; i < lng; i++) {
    ...
    if (low_key == high_key) {
      ...
      __ branch(lir_cond_equal, T_INT,
#ifdef RISCV64
                value,
      ...
      __ branch(lir_cond_lessEqual, T_INT,
#ifdef RISCV64
                value,
                LIR_OprFact::intConst(high_key),
#endif
...
void LIRGenerator::do_getEventWriter(Intrinsic* x) {
  ...
  __ branch(lir_cond_equal, T_OBJECT,
#ifdef RISCV64
            jobj,
  ...
  if (notify && (!backedge || UseOnStackReplacement)) {
    ...
    if (freq == 0) {
      if (!step->is_constant()) {
        ...
        __ branch(lir_cond_notEqual, T_ILLEGAL,
#ifdef RISCV64
                  step,
                  LIR_OprFact::intConst(0),
#endif
...
void LIRGenerator::do_RangeCheckPredicate(RangeCheckPredicate *x) {
  ...
  if (!a || StressRangeCheckElimination) {
    ...
    __ branch(lir_cond(cond), right->type(),
#ifdef RISCV64
              left, right,
#endif
              stub);
