...
void MacroAssembler::get_polling_page(Register dest, address page, relocInfo::relocType rtype) {
  if (SafepointMechanism::uses_thread_local_poll()) {
    ...
    int32_t off = 0;
    la_patchable(dest, Address(page, rtype), off);
...
// page.
address MacroAssembler::read_polling_page(Register r, int32_t offset, address page, relocInfo::relocType rtype) {
...
{
  assert(is_power_of_2_long(zero_words_block_size), "adjust this");
