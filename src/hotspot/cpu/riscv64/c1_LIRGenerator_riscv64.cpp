...
void LIRGenerator::do_ArithmeticOp_Long(ArithmeticOp* x) {
  ...
  if (x->op() == Bytecodes::_ldiv || x->op() == Bytecodes::_lrem) {
    ...
    if (need_zero_check) {
      ...
      __ branch(lir_cond_equal, T_LONG, right.result(), LIR_OprFact::longConst(0), new DivByZeroStub(info));
    }
...
void LIRGenerator::do_ArithmeticOp_Int(ArithmeticOp* x) {
  ...
  if (x->op() == Bytecodes::_idiv || x->op() == Bytecodes::_irem) {
    ...
    if (need_zero_check) {
      CodeEmitInfo* info = state_for(x);
      __ branch(lir_cond_equal, T_INT, right_arg->result(), LIR_OprFact::longConst(0), new DivByZeroStub(info));
    }
    ...
    if (right.is_constant()) {
      jint c = right.get_jint_constant();
      if (c > 0 && c < max_jint && (is_power_of_2_long(c) || is_power_of_2_long(c - 1) || is_power_of_2_long(c + 1))) {
        right_arg->dont_load_item();
      } else {
...
void LIRGenerator::do_If(If* x) {
  ...
  move_to_phi(x->state());
  if (x->x()->type()->is_float_kind()) {
    __ branch(lir_cond(cond), right->type(), left, right, x->tsux(), x->usux());
  } else {
    __ branch(lir_cond(cond), right->type(), left, right, x->tsux());
  }
